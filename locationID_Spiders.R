...
wd_shp <- "/gpfs1/data/idiv_meyer/00_data/original/GBIF/27_june_2022/External_data/External_files/Spiders_shapefile"

shp <- readOGR("Shapefile_spiders",dsn=wd_shp)  #load shapefile in
...
for(i in 1:ceiling(nrow(coords_sp)/1000000))  
{
  ...
  tab[[i]] <- data.frame(locationID=a$locationID,regSpiders=b$SpdrRgn) #file relating each locationID to a shapefile region
  print(i)
}
...
saveRDS(tab2,"locationID_regSpiders") #save locID_Spiderregions
