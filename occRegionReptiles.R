...
setwd("/gpfs1/data/idiv_meyer/01_projects/eduardo/GlobalAlienPatterns/Spiders")
...
table5 <- ddply(table4,.(species,year,regSpiders), nrow)

#save occurrences per region file
saveRDS(table5,"Spiders_occurrence_region_count")
...
#count observations per species per year per region
abs3 <- ddply(abs2,.(species,year,regSpiders), nrow)

#save absences per region file
saveRDS(abs3,"Spiders_absence_region_count")