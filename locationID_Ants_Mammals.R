...
wd_shp <- "/gpfs1/data/idiv_meyer/00_data/original/GBIF/27_june_2022/External_data/External_files/Bentity2_shapefile_fullres"

shp <- readOGR("Bentity2_shapefile_fullres",dsn=wd_shp)  #load shapefile in
...
for(i in 1:ceiling(nrow(coords_sp)/1000000))  
{
  ...
  tab[[i]] <- data.frame(locationID=a$locationID,BENTITY2_N=b$BENTITY2_N) #file relating each locationID to a shapefile region
  print(i)
}

tab2 <- rbindlist(tab)
names(tab2)[2] <- "regAntsMammals"

setwd("/gpfs1/data/idiv_meyer/00_data/original/GBIF/27_june_2022/External_data")

saveRDS(tab2,"locationID_regAntsMammals") #save locID_BENTITY2_N
