...
wd_list <- "/gpfs1/data/idiv_meyer/01_projects/eduardo/GlobalAlienPatterns/Ants"
wd_out <- "/gpfs1/data/idiv_meyer/01_projects/eduardo/GlobalAlienPatterns/Ants"

setwd(wd_list)
list <- read.csv("Final_checklist_ants.csv")
...
selectRecords <- function(list,sel.by="species",fields=NULL,original.data=TRUE,external.fields=NULL,
  ...
  for(i in 1:length(sel.by_files))
  {
    ...
    #gbifID_listID <- gbifID_listID[c(300000001:400000000),] #TEST DELETE
  ...
  if(!is.null(fields)){
    for(i in 1:length(fields))
    {
      ...
      for(j in 1:length(field_files))
      {
        ...
        #field_0[[j]] <- field_0[[j]][c(300000001:400000000),]
...
selectRecords(list = sps_list, wd.out = wd_out, file="Alien_ants_GBIF_occurrences",
