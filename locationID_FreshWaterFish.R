...
#coords_locID <- coords_locID[c(1:3000027),]  #just for test
 
wd_shp <- "/gpfs1/data/idiv_meyer/00_data/original/GBIF/27_june_2022/External_data/External_files/Simplified_FreshWater_shp"

shp <- readOGR("Basin042017_3119",dsn=wd_shp)  #load shapefile in
...
for(i in 1:ceiling(nrow(coords_sp)/1000000))  
{
  ...
  tab[[i]] <- data.frame(locationID=a$locationID, BasinName=b$BasinName) #file relating each locationID to a shapefile region
  print(i)
}

tab2 <- rbindlist(tab)
names(tab2)[2] <- "regFreshWaterFish"

setwd("/gpfs1/data/idiv_meyer/00_data/original/GBIF/27_june_2022/External_data")

saveRDS(tab2,"locationID_regFreshWaterFish") #save locID_BasinName
